<template>
    <div class="px-2 pb-3 fs14 d-flex align-items-end">
        <RouterLink to="/dashboard" class="text-tertiary-color text-decoration-none"> <i
                class=" bi bi-house-fill fs16 pe-1"></i>
            Dashboard</RouterLink>
        <span class="px-2">/</span>
        <div class="text-muted">Master Franchisee</div>
    </div>
    <div class="card">
        <div class="card-header py-3 bg-transparent">
            <div class="d-flex justify-content-between align-items-center">
                <div class="text-quaternary-color fs16 fw500">Master Franchisee</div>
                <div class="d-flex align-items-center" v-if="role === 'ADMIN'">
                    <ButtonOutlineTertiary @click="clients.length && exportAllDataToCSV()" v-if="clients.length > 0"
                        :disabled="exportLoading">
                        <span class="px-1">
                            Export <b class="bi bi-download ps-2 fs16"></b>
                        </span>
                    </ButtonOutlineTertiary>
                </div>
            </div>
        </div>
        <div class="card-body p-3">
             <div class="row mt-3 mb-3">
        <div class="col-md-6 col-12">
          <div class="text-quaternary-color fs14 fw500 mt-2">Total Clients: {{ totalClients }}</div>
        </div>
        <div class="col-md-6 col-12 text-end">
          <div class="d-flex align-items-center justify-content-end">
            <span class="text-secondary-color fs14 fw500 px-2">Entries Per Page</span>
            <select class="text-secondary-color fs14 w-auto fw500 form-select" v-model="entriesPerPage"
              @change="getMasterFranchisee">
              <option value="10">10</option>
              <option value="20">20</option>
              <option value="25">25</option>
              <option value="50">50</option>
              <option value="100">100</option>
            </select>           
          </div>
        </div>
      </div>
            <div v-if="!loading">
                <div v-if="clients.length > 0">
                    <div class="table-responsive pt-1">
                        <table class="table" style="width:auto">
                            <thead>
                                <tr>
                                    <th>Master Partner</th>
                                    <th>Remeshire Code</th>
                                    <th>Email ID</th>
                                    <th>Mobile No</th>
                                    <th>Master partner From Date</th>
                                    <th>Expiry Date</th>
                                    <th>Total Derivative First Trade Count</th>
                                    <th>Total Revenue</th>
                                    <th>Status</th>
                                    <!-- <th>50K Attain Date</th> -->
                                    <th>Partner From Date</th>
                                    <!-- <th>Max Date Among 10th Deri Given and 50K Given Date</th> -->

                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="(client, index) in clients" :key="index">
                                    <td>{{ client.MASTER_PARTNER || '-' }}</td>
                                    <td>{{ client.REMESHIRE_CODE || '-' }}</td>
                                    <td>{{ client.PARTNER_EMAIL_ID || '-' }}</td>
                                    <td>{{ client.PARTNER_MOBILE_NO || '-' }}</td>
                                    <td>{{ formatDate(client.MASTER_PARTNER_FROM_DATE) || '-' }}</td>
                                    <td>{{ formatDate(client.MASTER_PARTNER_EXPIRY_DATE) || '-' }}</td>
                                    <td>{{ client.TOTAL_DERI_FIRST_TRD_COUNT || '-' }}</td>
                                    <td>{{ client.TOTAL_REVENUE || '-' }}</td>
                                    <td>{{ client.ELIGIBLE_STATUS || '-' }}</td>
                                    <!-- <td>{{ formatDate(client.TOTAL_REV_50K_ATTAIN_DATE) || '-' }}</td> -->
                                    <td>{{ formatDate(client.PARTNER_FROM_DATE) || '-' }}</td>
                                    <!-- <td>{{ formatDate(client.MAX_DATE_AMONG_10TH_DERI_GIVEN_AND_50K_GIVEN_DATE) || '-' }}</td> -->

                                </tr>
                            </tbody>
                        </table>
                    </div>
                    <Pagination v-model="currentPage" :total-pages="totalPages" />
                </div>
                <div v-else>
                    <TableNoData class="py-5 my-md-3 my-xxl-4" />
                </div>
            </div>
            <div class="text-center my-5 py-5" v-else>
                <Loading class="my-5 py-4" />
            </div>
        </div>
    </div>
</template>

<script setup>
import { ref, onMounted, watch } from 'vue';
import Pagination from '../../../components/Pagination.vue';
import { format, addMinutes } from 'date-fns';
import { useAuthStore } from '@/stores/authStore';
import { tableService } from '../../../services/account/table.service';
import ButtonOutlineTertiary from '../../../components/button/ButtonOutlineTertiary.vue';
import TableNoData from '../../../components/TableNoDataFound.vue';
import { showErrorToast, showSuccessToast } from '../../../js/toast';
import Loading from '../../../components/Loading.vue';

const currentPage = ref(1);
const totalPages = ref(1);
const clients = ref([]);
const authStore = useAuthStore();
const adminDetails = ref({});
const errorMsg = ref('');
const totalClients = ref(0);
const loading = ref(false);
const entriesPerPage = ref(10);
const exportLoading = ref(false);
const role = ref(localStorage.getItem('role'));

onMounted(() => {
    adminDetails.value = authStore.adminDetails;
    getMasterFranchisee();
});

watch(currentPage, (newVal) => {
    getMasterFranchisee();
});

function formatDate(dateString) {
    if (!dateString) return null;
    const date = addMinutes(new Date(dateString), 5);
    //return format(date, "dd-MM-yyyy hh:mm a");
    return format(date, "dd-MM-yyyy");
}

const getMasterFranchisee = async () => {
    loading.value = true;
    const params = {
        remCode: adminDetails.value.RemCode,
        page: currentPage.value,
        size: entriesPerPage.value
    };
    try {
        const response = await tableService.getMasterFranchisee(params);
        if (response.data?.isSuccess) {
            clients.value = response.data?.data.result || [];
            totalPages.value = response.data?.data.noOfPages || 1;
            totalClients.value = response.data?.data.count;
        } else {
            clients.value = [];
            totalClients.value = 0;
            errorMsg.message = response.data?.message || 'Something went wrong, please try again later.';
        }
    } catch (err) {
        console.error(err);
        clients.value = [];
        totalClients.value = 0;
        errorMsg.message = 'Something went wrong, please try again later.';
    } finally {
        loading.value = false;
    }
};

const exportAllDataToCSV = async () => {
    exportLoading.value = true;
    try {
        const params = {
            remCode: adminDetails.value.RemCode,
            page: 1,
            size: totalClients.value,
        };
        const response = await tableService.getMasterFranchisee(params);
        if (response.data?.isSuccess && response.data.data.result.length > 0) {
            const allClients = response.data.data.result;

            const headers = ["Master Partner", "Remeshire Code", "Email ID", "Mobile No", "Master Partner From Date", "Expiry Date",
                "Total Derivative First Trade Count", "Total Revenue", "Status", "Partner From Date"];

            const rows = allClients.map(client => [
                client.MASTER_PARTNER || '-',
                client.REMESHIRE_CODE || '-', 
                client.PARTNER_EMAIL_ID || '-',
                client.PARTNER_MOBILE_NO || '-',
                formatDate(client.MASTER_PARTNER_FROM_DATE) || '-',
                formatDate(client.MASTER_PARTNER_EXPIRY_DATE) || '-',
                client.TOTAL_DERI_FIRST_TRD_COUNT || '-',
                client.TOTAL_REVENUE || '-',
                client.ELIGIBLE_STATUS || '-',
                // formatDate(client.TOTAL_REV_50K_ATTAIN_DATE) || '-',
                formatDate(client.PARTNER_FROM_DATE) || '-',
                // formatDate(client.MAX_DATE_AMONG_10TH_DERI_GIVEN_AND_50K_GIVEN_DATE) || '-'

            ]);

            let csvContent = "data:text/csv;charset=utf-8," + [headers.join(","), ...rows.map(row => row.join(","))].join("\n");

            const encodedUri = encodeURI(csvContent);
            const link = document.createElement("a");
            link.setAttribute("href", encodedUri);
            link.setAttribute("download", "Master Franchisee Details - All Data.csv");
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);

            showSuccessToast("All Master Franchisee Data Exported Successfully");
        } else {
            showErrorToast("No data available to export.");
        }
    } catch (error) {
        console.error(error);
        showErrorToast("Error while exporting data.");
    } finally {
        exportLoading.value = false;
    }
};


</script>

<style scoped lang="scss">
.no-data {
    width: 400px !important;
}

@media(max-width:1400px) {
    .no-data {
        width: 360px !important;
    }
}

@media(max-width:767px) {
    .no-data {
        width: 300px !important;
    }
}

@media(max-width:1400px) {
    .table {
        width: 1600px !important;
    }
}

.table-responsive {
    overflow-x: auto;
}

.table {
    min-width: 1900px; 
}

.form-control,
.form-select {
    height: 40px !important;
}
</style>
