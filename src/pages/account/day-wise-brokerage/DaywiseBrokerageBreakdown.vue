<template>
    <div class="modal fade" id="DaywiseBrokerageBreakdown" tabindex="-1" aria-labelledby="DaywiseBrokerageBreakdown" aria-hidden="true"
...
watch(
  () => [props.remCode, props.date, currentPage.value],
  () => {
    if (props.remCode && props.date) {
      getDayWiseBrokerageBreakdown();
    }
  },
  { immediate: true }
);
...
const getDayWiseBrokerageBreakdown = async () => {
  const params = {
    remCode: props.remCode,
    date: props.date,
    page: currentPage.value,
  };

  try {
    const response = await tableService.getDayWiseBrokerageBreakDown(params);
...
const closeModal = () => {
    const modalElement = document.getElementById("DaywiseBrokerageBreakdown");
